...
beg_date = datetime.datetime.strptime('20100201', '%Y%m%d')
end_date = datetime.datetime.strptime('20100202', '%Y%m%d')
...
data_root = '/pscratch/sd/z/zhang73/DATA/data_hiccup/'

# Specify output atmosphere horizontal grid
dst_horz_grid = 'mono_CAx32v1pg2'   

# Specify output atmosphere vertical grid
dst_vert_grid,vert_file_name = 'L128',f'/pscratch/sd/z/zhang73/DATA/data_hiccup/vrt_coord_scream_L128.nc'

# local path for grid, map, and data files
src_data_root = f'/pscratch/sd/z/zhang73/DATA/data_hiccup/ERA5_pres_newcds/2010/'
dst_data_root = f'/pscratch/sd/z/zhang73/DATA/data_hiccup/{dst_vert_grid}.{dst_horz_grid}.UVTQ.pres.cdsnew.240929/'
...
def get_nudge_file(datetime):
    return f'{dst_data_root}/HICCUP.atm_era5.{datetime}.{dst_horz_grid}.{dst_vert_grid}.nc'
# ------------------------------------------------------------------------------
...
for t in datetime_list:
    ...
    nudge_datetime = t.strftime("%Y%m%d_%H")

    src_atm_file = f'{src_data_root}/ERA5.uvtqz.{nudge_datetime}.nc'
    src_sfc_file = f'{src_data_root}/ERA5.ps.{nudge_datetime}.nc'
    ...
    if create_map_file and not map_file_created:
        ...
        hiccup_data.src_grid_file = f'/pscratch/sd/z/zhang73/DATA/data_hiccup/ERA5_721x1440_scrip.20220907.nc'
        hiccup_data.dst_grid_file = f'/global/homes/z/zhang73/grids/TempestRemap_Out/CA_ne32_x32_v1_pg2.g'
        hiccup_data.map_file      = f'/pscratch/sd/z/zhang73/grids2/CAne32x32v1/map_ERA5_721x1440_to_CA_ne32_x32_v1_pg2_nco.fv2fv_flx_mono.20220907.nc'
    # --------------------------------------------------------------------------
    ...
    if add_case_t0:
        ...
        cmd = f'ncatted -O -a units,time,o,c,\'hours since {ref_date} 00:00:00\' -a units,time_bnds,o,c,\'hours since {ref_date} 00:00:00\' {output_atm_file_name}'
        run_cmd(cmd,verbose,shell=True)
        cmd = f'ncatted -O -a _FillValue,,o,f,3.402824e+33 -a _FillValue,time,d,, -a _FillValue,time_bnds,d,, {output_atm_file_name}'
        run_cmd(cmd,verbose,shell=True)
# ------------------------------------------------------------------------------
